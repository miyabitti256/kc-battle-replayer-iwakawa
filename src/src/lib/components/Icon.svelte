<script lang="ts">
	// Why not external icon library: Self-contained zero-dependency SVG avoids package weight and ensures consistent stroke styling.
	type IconName =
		| 'folder'
		| 'folder-open'
		| 'file-text'
	...
	let { name, class: className = 'size-3.5', size = 14 }: Props = $props();
</script>
